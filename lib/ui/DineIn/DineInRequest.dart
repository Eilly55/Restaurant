...
import 'package:uber_eats_restaurant_dashboard/ui/DineIn/HistoryTableBooking.dart';
import 'package:uber_eats_restaurant_dashboard/ui/DineIn/UpComingTableBooking.dart';
...
class _DineInRequestState extends State<DineInRequest> {
  ...
  void getcurcy() {
    ...
    futureCurrency.then((value) {
      for (int i = 0; i < value.length; i++) {
        CurrencyModel currencyModel = value[i];

        if (currencyModel.isactive) {
          symbol = currencyModel.symbol;
          isRight = currencyModel.symbolatright;
          decimal = currencyModel.decimal;
        }
      }
    });
  }
        ...
        child: Column(
          ...
          children: [
            ...
            buildDetails(
                ...
                title: 'Name'.tr(),
                value: "${bookTableModel.author.lastName} ${bookTableModel.author.lastName}"),
            ...
            buildDetails(
                ...
                title: 'Date'.tr(),
                value: "${DateFormat("MMM dd, yyyy 'at' hh:mm a").format(bookTableModel.date.toDate())}"),
                      ...
                      child: Container(
                          ...
                          child: Center(
                            child: Text(
                              'Accept'.tr(),
                              style: TextStyle(
                                  color: isDarkMode(context) ? Color(0xffFFFFFF) : Color(COLOR_PRIMARY),
                                  fontFamily: "Poppinsm",
                                  fontSize: 15
                  ...
                  child: Padding(
                    ...
                    child: InkWell(
                      child: Container(
                          padding: EdgeInsets.only(top: 8, bottom: 8),
                          decoration:
                              BoxDecoration(borderRadius: BorderRadius.circular(5), border: Border.all(width: 0.8, color: Colors.grey)),
                          child: Center(
                            child: Text(
                              'Rejected'.tr(),
